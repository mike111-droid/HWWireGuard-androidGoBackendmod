...
func (device *Device) RoutineHandshake(id int) {
	...
	for elem := range device.queue.handshake.c {
		...
		case MessageInitiationType:
			...
			device.log.Verbosef("%v - Received handshake initiation (try %d)", peer, peer.timers.actualHandshakeAttempts)
			/* Custom change begin */
			//device.log.Verbosef("%v - Handshake attempts: %d", peer, peer.timers.actualHandshakeAttempts)
                        atomic.AddUint32(&peer.timers.actualHandshakeAttempts, 1)
			//device.log.Verbosef("%v - Handshake attempts: %d", peer, peer.timers.actualHandshakeAttempts)
                        /* Custom change end */
