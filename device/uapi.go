	...
	func() {
		...
		for _, peer := range device.peers.keyMap {
			...
			func() {
				...
				/* Custom change begin */
				sendf("handshakeAttempts=%d", peer.timers.actualHandshakeAttempts)

				keyf("chainKey", (*[32]byte)(&peer.handshake.sharedUniqueSecret))
				//sendf("chainKey=%s", (*[32]byte)(&peer.handshake.remoteEphemeral))
				//device.log.Verbosef("%v - chainKey: %s", peer, string(peer.handshake.chainKey[:]))
				//device.log.Verbosef("%v - chainKey: %s", peer, string(peer.handshake.remoteEphemeral[:]))
				/* Custom change end */
...
/* Custom change begin */
func (device *Device) IpcSetPSK(uapiConf string) error {
	return device.IpcSetPSKOperation(strings.NewReader(uapiConf))
}

func (device *Device) IpcSetPSKOperation(r io.Reader) (err error) {
	// Lock device
	device.ipcMutex.Lock()
	defer device.ipcMutex.Unlock()
	defer func() {
	...
	for scanner.Scan() {
		...
		// loadPSK if line is preshared_key
		if key == "preshared_key" {
			if peer == nil {
				return ipcErrorf(ipc.IpcErrorInvalid, "failed to find peer: %w", err)
			}
	        	peer.handshake.mutex.Lock()
		        err := peer.handshake.presharedKey.FromHex(value)
		        peer.handshake.mutex.Unlock()
